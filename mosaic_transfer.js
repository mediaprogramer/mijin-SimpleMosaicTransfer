...
//        temporary.push({'entity':entity[j], 'value':mosaicIdToName(entity[j].mosaicId) + " : " + entity[j].quantity})
        temporary.push({'entity':entity[j], 'value':MOSAIC_INFO + " : " + entity[j].quantity})
    }
...
function mosaicTransferRequest(){
	...
	var numNem = calcXemEquivalent(SEND_MULTIPLIER * 1000000, SEND_QUANTITY, MOSAIC_SUPPLY, MOSAIC_DIVISIBILITY);
	var fee = CALC_MIN_FEE(numNem) * 5 / 4;
...
//　ここでいったん想定内の手数料か確認することをオススメします。
//	return false;

	var result = confirm("手数料は" + totalFee.toString() + "です（下6桁は小数桁）。間違って送信した場合、戻りません。");
	if(!result){
		return false;
	}
